...
/*
 * Shared helper for locking a file. Caller uses jtr_lock(fd, type, mode)
 * which is actually a macro adding __FUNCTION__, __FILE__ and __LINE__ to
 * params.
 */
int real_lock(int fd, int type, int mode,
              const char *function, const char *file, int line)
{
	...
	fprintf(stderr, "%s(%u): Locking %s...\n",
	        function, options.node_min, f->name);
#endif

#if FCNTL_LOCKS
	struct flock lock;
	int cmd = mode ? F_SETLKW : F_SETLK;

	memset(&lock, 0, sizeof(lock));
	lock.l_type = type ? F_WRLCK : F_RDLCK;
	while (fcntl(fd, cmd, &lock))
#else
	int cmd = type ? LOCK_EX : LOCK_SH;
...
#endif
	{
		if (errno == EAGAIN) {
			...
			if (!warned++) {
				log_event("Got EAGAIN despite F_SETLKW (only logged once per node) %s:%d", file, line);
...
/*
 * Shared helper for releasing a lock. Caller uses jtr_unlock(fd) which is
 * actually a macro adding __FUNCTION__, __FILE__ and __LINE__ to params.
 */
void real_unlock(int fd, const char *function, const char *file, int line)
{
#if OS_FLOCK || FCNTL_LOCKS
